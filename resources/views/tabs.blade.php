                                            ...
                                            <div class="rate text-right">
                                                <ul>
                                                    ...
                                                    @if($c->discount_price == !NULL)
                                                        ...
                                                        <li><a><b>{{ activeCurrency()->symbol }}{{ price_format( currency($c['discount_price'], $from = $currency->code, $to = Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code, $format = false)) }}</b></a></li>

                                                        <li><a><b><strike>{{ activeCurrency()->symbol }}{{ price_format(  currency($c['price'], $from = $currency->code, $to = Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code, $format = false)) }}</strike></b></a></li>
                                                       
                                                        
                                                    @else

                                                        @if($c->price == !NULL) 
                                                        <li><a><b>{{ activeCurrency()->symbol }}{{ price_format(  currency($c['price'], $from = $currency->code, $to = Session::has('changed_currency') ? Session::get('changed_currency') : $currency->code, $format = false)) }}</b></a></li>
                                                        @endif
                                            ...
                                            @else
                                                <div class="protip-btn">
                                                    ...
                                                    <form id="demo-form2" method="post" action="{{ route('guest.addtocart', $c->id) }}"
                                                        ...
                                                        <div class="box-footer">
                                                         <button type="submit" class="btn btn-primary shopping-cart"><i data-feather="shopping-cart"></i>&nbsp;{{ __('frontstaticword.AddToCart') }}</button>
                                                        </div>
                                                    </form>

                                                    @else

                                                    <a href="{{ route('login') }}" class="btn btn-primary shopping-cart"><i data-feather="shopping-cart"></i>&nbsp;{{ __('frontstaticword.AddToCart') }}</a>
